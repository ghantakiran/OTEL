...
- **The Agent stays core-only.** An Agent runs beside a service, on whatever disk that service has, and its next hop is the Gateway — which is inside the cluster and, unlike a Backend, something this platform operates. `Delivery.Spill` exists on the type a Profile and a Backend share, so a Pipeline Profile setting it is **refused when the Profiles are loaded** rather than silently ignored: a Profile that reads as durable while the fleet runs an in-memory queue is exactly the setting-nobody-reads failure this repository refuses elsewhere.
...
- **Spill declared per Backend as a full directory path** rather than derived from `spill_root` plus the Backend's name. More flexible — an archive's spill could sit on cheaper disk — but two Backends can then be given the same path by a typo, silently sharing a storage instance and reintroducing the coupling above. Deriving makes that unrepresentable, but only because the derivation is closed on both ends: `spill_root` must be absolute, and a Backend's name must match `^[a-z0-9]([a-z0-9-]*[a-z0-9])?$`. Without that charset the claim would be false rather than merely weak — a name is a path segment and a collector component ID, and both normalise, so `./a` and `a` would clean to one directory and `" primary-apm"` would be trimmed into an existing exporter, each defeating the duplicate-name check by looking distinct to it.
